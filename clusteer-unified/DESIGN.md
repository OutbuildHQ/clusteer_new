...
Scrolling changes presentation only. Direction, amount, asset, network and receipt survive reversal and local dashboard navigation. Scroll position always controls the scene, including after using the converter. When scrolling back past the card’s interactive range, focus moves to the scene without changing the scroll position; all conversion selections remain intact. Hidden dashboard and in-flight card controls are inert; arrival actions move focus without scrolling the document unexpectedly. The normal page scroll continues below the scene. Smaller screens and reduced motion use explicit dashboard/conversion controls and ordinary document flow. Switching between these modes preserves an open converter, and ordinary mobile viewport resizes do not dismiss it.
...
Public focus styling uses a single 2px keyboard indicator on actions. Pointer clicks do not draw action outlines. Amount groups use their existing border for active-field feedback; converter selectors use an inset underline for keyboard focus. Waitlist fields strengthen their border without an external ring. Programmatic focus on the laptop region never outlines the entire frame. Dashboard focus styles remain scoped to their existing workspace system.
...
## Website dropdowns — 15 September 2026

The hero asset and network selectors use a dedicated WebsiteSelect backed by the existing Radix Select dependency. The trigger retains compact logos and labels; the menu provides 64px option rows, full names, secondary asset/network descriptions and a selected checkmark. Menus portal outside the transformed hero so they are not clipped, stay within viewport edges and return focus on dismissal. Keyboard selection, typeahead and Escape use Radix behavior. Motion is limited to a 150ms entrance and respects reduced motion. Authenticated dashboard primitives and website navigation retain their existing layouts.
