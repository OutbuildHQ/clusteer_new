	...
	return (
		...
		>
			<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
				...
				{copied ? (
					<Check size={14} style={{ color: "var(--c-up)" }} />
				) : (
					<Copy size={14} style={{ color: "var(--c-text-3)" }} />
				)}
			</div>
			<span
				style={{
		...
		>
			...
			<span
				style={{
					fontSize: 12,
					fontWeight: 600,
					color: urgent ? "var(--c-down)" : "var(--c-text-2)",
				}}
			>
				Preview expires in
			</span>
			<span
				style={{
					...
					fontWeight: 700,
					fontSize: 14,
					color: urgent ? "var(--c-down)" : "var(--c-text)",
...
export function SellHandoff({ order, onDone, onCancel }: Props) {
	...
	const networkName =
		dd.chain === "TRC20" ? "Tron" : dd.chain === "BEP20" ? "BNB Smart Chain" : "Ethereum";
		...
		<div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
			...
			>
				...
				<p style={{ fontSize: 12, color: "var(--c-text)", lineHeight: 1.5, margin: 0 }}>
					Send <strong>only USDT</strong> on the{" "}
					<strong>
						{networkName} ({chainLabel})
					</strong>{" "}
					network. Sending any other token or using the wrong network will result in permanent loss.
				</p>
			</div>
			...
			>
				<Info size={14} style={{ color: "var(--c-up)" }} />
				<span style={{ fontSize: 11, color: "var(--c-text-3)" }}>
					Illustrative instructions. No payment is processed.
				</span>
			</div>
