...
export function SellEntry({ state, updateState, onContinue, onSwitchSide }: Props) {
	const [usdtAmount, setUsdtAmount] = useState(state.amount > 0 ? String(state.amount) : "");
	...
	const selectedBank = banks?.find(
		(b: { bankCode: string; accountNumber: string; bankName: string }) =>
			b.bankCode === bankCode && b.accountNumber === accountNumber
	);
				...
				<div>
					...
					{banks && banks.length > 0 ? (
						...
						<div className="mt-1 py-2.5 px-3 border border-ds-line rounded-xl flex justify-between items-center">
							...
							<a
								href="/settings"
								className="text-[12px] font-semibold text-onyx-900 dark:text-lime-500 no-underline"
							>
								Add bank
							</a>
						</div>
