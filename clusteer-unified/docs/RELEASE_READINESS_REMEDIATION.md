...
**Auth bridge — decided (2026-07-07): build it. Built, audited twice, fixes applied both rounds.** Design chosen after reading Spring's actual login contract (`UserController.java:30-34` → `UserServiceImpl.loginUser`, `models/dtos/user/Login.java`: `{email, password}` → `HttpResponse{responseData: {accessToken, data}}`) and confirming the constraint that ruled out the simpler alternatives: `auth_token` can't just be swapped for a Spring JWT (Firebase-gated `middleware.ts` and `api-helpers.ts`'s `getUserIdFromToken` both assume its subject is a Firebase UID), and Spring's own signature check (HS256, its own secret) means you can't mint a compatible token without literally calling Spring's login endpoint — a hand-minted JWT would also fail `JwtService.isTokenValid()`'s `security.token`-must-match-DB-row check.

- [x] **`springLogin()` / `getSpringTokenFromRequest()` / `isSpringBridgeBlocked()`** (`packages/ui/src/lib/spring-boot-server.ts`) — calls Spring's real `POST /v1/user/login` with the same email/password the user just used for Firebase. `springLogin` distinguishes three outcomes: `"not_found"` (400, no Spring account for this email), `"rejected"` (any other non-ok response — a genuine login failure, e.g. wrong password), `"network_error"` (never got a clean parseable response at all — safe to retry later). Only `"rejected"` is treated as permanent.
- [x] **`auth-firebase/login/route.ts`** — after a successful Firebase login, calls `springLogin` **only if** this browser has neither an existing `spring_session` cookie nor a `spring_bridge_blocked` cookie (see the round-1 audit fix below for why unconditional calling was dangerous). Success sets `spring_session` (httpOnly, 30-day maxAge). A `"rejected"` result sets `spring_bridge_blocked` (httpOnly, 30-day maxAge) so this browser never auto-retries. Everything here is best-effort/non-fatal — the user's Firebase session is unaffected either way.
- [x] **`auth-firebase/logout/route.ts`** — also calls Spring's real `PUT /v1/user/logout` (clears `security.token` server-side) and deletes `spring_session`. A `logoutFirebase()` failure is caught locally and no longer skips this or the cookie clearing.
- [x] **`auth-firebase/invalidate-sessions/route.ts`** (the post-password-reset flow from Module D) — also deletes `spring_session`, since a Firebase-only reset leaves Spring's stored password hash stale (see the open gap below) and the old bridge token shouldn't be trusted going forward.
- [x] **`api/user/api-keys/route.ts` + `[id]/route.ts`** — use `spring_session` instead of the Firebase token; return a clear `409` if the user has no Spring session, instead of sending a token that would fail Spring's filter with an opaque error.
...
**Audit round 1 (`ec9b376` → `f38e58a`) — 1 critical bug, 1 high, both fixed:**

1. **CRITICAL, fixed: guaranteed account lockout.** The first version called `springLogin` on *every* Firebase login, including the frequent re-logins `auth_token`'s 1-hour expiry forces during normal use. Spring's failed-attempt counter (`AccountSecurity.signInAttempts`) only resets on a *successful* Spring login — never on a timer — so after any Firebase-only password reset (Spring's hash goes stale, see the open gap below), *every* subsequent login was a failed Spring attempt. Three logins in and Spring auto-deactivates the account and emails the user a "SECURITY BREACH" notice, with no self-service recovery. Not a hypothetical retry-loop — a guaranteed consequence of continuing to use the app normally after any password reset. Fixed by the "only attempt if no existing session and not already blocked" logic described above, plus splitting `springLogin`'s old single `"error"` reason into `"rejected"` (don't retry) vs `"network_error"` (safe to retry) so a transient failure can't get permanently blocked, and a real rejection can't get retried into a lockout.
2. **HIGH, fixed: cookie-name collision.** The original cookie, `spring_auth_token`, contained "auth_token" as a substring. `api-helpers.ts`'s `getAuthFromRequest` used an unanchored regex (`/auth_token=([^;]+)/`) to read the *real* `auth_token` (Firebase) cookie — empirically confirmed (Node repro) that a request carrying only a forged `spring_auth_token` cookie passed the primary Firebase auth check, and that cookie ordering could make the Spring token get read as the Firebase token. Fixed by renaming to `spring_session` (zero substring overlap with any cookie name in use) and anchoring both regexes to `(?:^|;\s*)name=` so no cookie name can be mistaken for a suffix match of another.

**Audit round 2 (`f38e58a`) — re-checked the round-1 fix itself, found one more real bug plus residual risks worth stating plainly rather than silently accepting:**

1. **MEDIUM, fixed (`2026-07-07`, same day):** the identical unanchored-regex bug the round-1 fix closed for `auth_token`/`spring_session` was still live in two **admin app** routes the fix's scope missed — `apps/admin/src/app/api/fees/route.ts:14` and `apps/admin/src/app/api/orders/route.ts:31`, both hand-rolling `cookieHeader.match(/admin_token=([^;]+)/)` instead of `request.cookies.get("admin_token")`. Not exploitable today (no cookie in the current namespace is a superstring of `admin_token`), but the same vulnerability class, left live on the endpoints guarding admin fee-editing and order-listing. Fixed both to use `request.cookies.get("admin_token")?.value` (Next.js's built-in exact-name cookie API — safe by construction, matching what `admin-auth.ts` already does correctly). Grepped the whole monorepo afterward for any remaining unanchored `*token*=` cookie regex — zero hits.
2. **Residual risk, accepted, not fixed — stated explicitly per the audit's own ask, not swept under "mitigated":** the block is per-browser (cookie-based). A user with 2-3 distinct browsers/devices, or who clears cookies, can still rack up multiple genuine rejections across them — realistically reachable, not just theoretical, though far better than the round-1 design's guaranteed single-device lockout. There is currently no way to reset a browser's `spring_bridge_blocked` state on purpose (no UI, no endpoint) — this is coupled to the "password drift" gap below; a real fix for one would likely fix both.
3. **Residual gap, accepted, not fixed:** Spring keeps exactly one valid token per account (`AccountSecurity.token`, overwritten by any new login). Logging into Spring from a second device silently invalidates the first device's cached `spring_session` — and since login only calls `springLogin` when *no* session cookie is present (not when the existing one is verified live), the first device keeps skipping the call and silently fails Spring-backed calls until an explicit logout/login. Low blast radius today (only `api-keys` consumes the bridge token).
4. **Residual gap, accepted, not fixed:** Spring's `GlobalExceptionHandler` maps wrong-password (`ApplicationException`), a locked account (`AccessDeniedException`), *and* genuinely unrelated server errors (a DB blip, an NPE) to the identical 500 with an identical generic message — there is no way to tell them apart from the HTTP response alone. `springLogin` therefore treats any non-400 failure as `"rejected"` (permanent), which means a real Spring-side outage could wrongly and permanently block a browser. This is the safer of the two possible defaults: the alternative (treat unclear 500s as retryable) reintroduces the round-1 lockout bug for the far more common "actually wrong password" case. Deliberate tradeoff, not an oversight — but only closeable for real with a Spring-side change (a distinct status/code for auth failures), not more Next.js code.
5. **Residual gap, accepted, not fixed:** two near-simultaneous login requests from the same browser (double-submit, two tabs) both read "no cookie yet" and could both call `springLogin`, each independently consuming one of Spring's 3 attempts for a single logical login. Partially mitigated by the existing `rateLimit(request, RateLimitPresets.strict)` already on this route; not fully closed.

**Known gaps, not fixed — need a decision, not more code from me right now:**

- **Password drift after a Firebase-only reset.** Spring only exposes two password-write paths (`UserServiceImpl.java`): `changePassword` (needs the *old* password) and `resetPassword` (needs a token Spring itself emails to the user — not something Next.js can drive server-to-server). There is no admin/service-credential override. So the first time any user resets their password through the existing "forgot password" flow, Spring's hash silently goes stale and `springLogin` starts returning `"rejected"` for them from then on — correctly non-retried after round 1's fix, but permanently broken for that browser until someone builds a real fix. The only clean options are: (a) add a new Spring endpoint, callable only via the existing `X-API-KEY` service credential (not user-facing), that lets Next.js push a password sync after a *verified* Firebase-side change — this means editing the Java backend, a bigger step I didn't take without asking; or (b) trigger Spring's own emailed reset flow in parallel, which means two separate reset emails per user, worse UX. Flagging for a decision rather than guessing.
