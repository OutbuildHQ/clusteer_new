...
- [?] `apps/customer/src/app/api/auth-firebase/login/route.ts:1-92` — never calls Spring's real `POST /v1/user/login`; login is 100% Firebase-only today, so the backend never learns about a login event or syncs profile/roles. **Confirmed 2026-07-07 this is one symptom of a broader gap — see the blocking architectural finding above.** Decide whether the auth bridge matters for the current architecture, and build it if so — this now also blocks Module E's API-keys feature and the (currently unreachable) trade/OTP routes, not just login-sync.
