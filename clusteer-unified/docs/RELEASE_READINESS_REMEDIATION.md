...
### Tasks (SAFE-TO-FIX) — RESOLVED (2026-07-03)

- [x] `apps/customer/src/app/(marketing)/buy/page.tsx:94-101` — removed the "LIVE" badge/dot, changed copy to "SAMPLE" + "illustrative, live rate shows in the app" (matches the disclaimer pattern already used on `rate-alerts/page.tsx`). Did not wire a real fetch here — that's a bigger feature change than a honesty pass; the illustrative-label approach was chosen instead.
- [x] `apps/customer/src/app/(marketing)/buy/page.tsx:136,159` — labeled "Order ticket — example"; softened "Settles instantly" + live-dot to "Settles on-chain" (no animation implying real-time).
- [x] `apps/customer/src/app/(marketing)/sell/page.tsx:80-121` — labeled "Sell ticket — example", same live-dot/instant softening. The fabricated "5 min" stat replaced with "100% / Non-custodial — your funds are always yours" (a real, already-established claim reused from `auth-accent-panel.tsx`'s own honest stats).
