...
import { Plus, Download, ChevronRight, TrendingUp, Wallet as WalletIcon } from "lucide-react";
...
export default function AssetsPage() {
	...
	const totalNgn = allRows.reduce((s, a) => s + a.balanceNgn, 0);

	const displayRows = tab === "Holdings"
		? allRows.filter(a => a.balance > 0)
		: allRows;
		...
		<div className="space-y-5">
			...
			<div className="grid grid-cols-1 sm:grid-cols-[2fr_1fr] gap-3">
					...
					<div>
						...
						{isLoading ? (
							...
							<div className="mt-2 font-display tabular-nums text-[30px] lg:text-[42px] font-semibold leading-none">
								{formatMoney(totalNgn, "NGN", { decimals: 0 })}
							</div>
						)}
				...
				<div className="ds-card p-5">
					...
					{isLoading ? (
						<div className="animate-pulse rounded-[10px] w-36" style={{ background: "var(--c-surface-3)", height: 30 }} />
					) : (
						<div className="font-display tabular-nums text-[30px] font-semibold leading-none text-[var(--c-text)]">
							{formatMoney(ngnBalance, "NGN", { decimals: 0 })}
						</div>
					)}
				...
				{isLoading ? (
					...
					<div className="py-16 text-center px-4 bg-grid">
						<div className="size-14 rounded-2xl inline-flex items-center justify-center mb-4 bg-[var(--c-lime-500)]">
							<WalletIcon className="size-6 text-[var(--c-onyx-900)]" />
						</div>
							...
							<thead>
								<tr className="border-b border-[var(--c-line)] bg-[var(--c-surface-2)]">
									{["Asset", "Balance", "Value", ""].map((h, i) => (
										<th
											key={i}
											className={`px-4 py-3 text-left text-[11.5px] font-medium uppercase tracking-[0.05em] text-[var(--c-text-3)] ${i === 2 ? "text-right" : ""}`}
										>
											{h}
										</th>
									))}
										...
										<td className="px-4 py-3">
											<Link href={`/assets/${a.symbol}`} className="flex items-center gap-2.5">
												...
												<div>
													...
													<div className="text-[11px] text-[var(--c-text-3)]">{a.chain}</div>
												</div>
											</Link>
										</td>
					...
					<div className="lg:hidden">
						{displayRows.map((a, i) => (
							<Link
								...
								style={{ borderBottom: i < displayRows.length - 1 ? "1px solid var(--c-line)" : "none" }}
							>
