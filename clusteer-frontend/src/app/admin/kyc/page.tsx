...
export default function AdminKyc() {
	...
	const current = KYC_QUEUE.find((k) => k.id === open);
	return (
		<div className="space-y-6">
			<div>
				<h1 className="font-display text-2xl font-bold tracking-tight">KYC review queue</h1>
				<p className="text-sm text-muted-foreground">{KYC_QUEUE.length} submissions awaiting review</p>
			</div>
						...
						<>
							...
							<div className="space-y-4">
								...
								<div>
									...
									<Input id="note" className="mt-1.5" placeholder="Optional note for audit log" />
								</div>
							</div>
							<DialogFooter className="gap-2 sm:justify-between">
								<Button variant="destructive" onClick={() => { toast.error("Submission rejected"); setOpen(null); }}><X className="size-4" />Reject</Button>
								<div className="flex gap-2">
									<Button variant="outline" onClick={() => setOpen(null)}>Cancel</Button>
									<Button onClick={() => { toast.success("Tier " + current.tier + " approved"); setOpen(null); }}><Check className="size-4" />Approve</Button>
								</div>
							</DialogFooter>
						</>
					)}
