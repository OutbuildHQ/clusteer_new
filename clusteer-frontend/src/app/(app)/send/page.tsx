...
export default function SendPage() {
	...
	const FEES: Record<string, number> = { Tron: 1, BSC: 0.5, Ethereum: 5, Solana: 0.01 };
	const fee = FEES[chain] ?? 1;
	...
	return (
		<div className="max-w-2xl mx-auto">
			<h1 className="font-display text-2xl font-bold tracking-tight mb-6">Send crypto</h1>
			<Card>
				<CardHeader>
					<StepsHorizontal
				...
				<CardContent className="space-y-5">
					{step === "form" && (
						<>
							...
							<div>
								...
								<Input id="address" className="mono mt-1.5" placeholder={chain === "Tron" ? "T… (Tron / TRC20 address)" : chain === "Solana" ? "8v… or base58 Solana address" : "0x… (EVM address)"} value={address} onChange={(e) => setAddress(e.target.value)} />
							</div>
					...
					{step === "review" && (
						<div className="space-y-4">
							<div className="rounded-lg border border-border p-4">
								...
								<Num as="div" className="mt-1 font-display text-3xl font-bold" value={amount + " " + asset} />
					...
					{step === "otp" && (
						<div className="space-y-4">
							...
							<Input className="mono text-center text-2xl tracking-widest" maxLength={6} placeholder="000000" value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, ""))} />
							<div className="flex gap-2">
